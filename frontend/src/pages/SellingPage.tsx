// src/pages/SellingPage.tsx
import React, { useEffect, useMemo, useState } from "react";
import { Listing } from "../types";
import { apiGet, apiPatch } from "../api";
import { useUser } from "../UserContext";

const buyerNameMap: Record<string, string> = {
  alice: "Alice",
  bob: "Bob",
  charlie: "Charlie",
};

const defaultImage =
  "data:image/png;base64,";

const fallbackAvatar =
  "https://img.daisyui.com/images/stock/photo-1534528741775-53994a69daeb.webp";

const SellingPage: React.FC = () => {
  const { currentUserId, users } = useUser();
  const [listings, setListings] = useState<Listing[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Details 弹窗
  const [selectedListing, setSelectedListing] = useState<Listing | null>(null);

  // 通用成功弹窗（Cancel / Sold 共用）
  const [showActionSuccessModal, setShowActionSuccessModal] = useState(false);
  const [lastAction, setLastAction] = useState<"cancel" | "sold" | null>(null);
  const [actionListingTitle, setActionListingTitle] = useState<string | null>(
    null
  );

  // sellerId -> User 的 map（虽然这里都是 currentUser，但保持统一）
  const userMap = useMemo(() => {
    const map: Record<string, (typeof users)[number]> = {};
    for (const u of users) {
      map[u.id] = u;
    }
    return map;
  }, [users]);

  const getSellerDisplay = (sellerId: string) => {
    const seller = userMap[sellerId];
    const name = seller?.name || sellerId;
    const avatarUrl = seller?.avatarUrl || fallbackAvatar;
    return { name, avatarUrl };
  };

  const loadListings = async () => {
    try {
      setLoading(true);
      setError(null);
      const data: Listing[] = await apiGet(
        `/api/listings?sellerId=${encodeURIComponent(currentUserId)}`
      );
      setListings(data);
    } catch (err: any) {
      setError(err.message || "Failed to load your listings");
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    loadListings();
  }, [currentUserId]);

  const handleSell = async (id: string, title: string) => {
    try {
      await apiPatch(`/api/listings/${id}/sell`, {});
      await loadListings();

      setLastAction("sold");
      setActionListingTitle(title);
      setShowActionSuccessModal(true);
    } catch (err: any) {
      alert(err.message || "Failed to mark as sold");
    }
  };

  const handleCancel = async (id: string, title: string) => {
    try {
      await apiPatch(`/api/listings/${id}/cancel`, {});
      await loadListings();

      setLastAction("cancel");
      setActionListingTitle(title);
      setShowActionSuccessModal(true);
    } catch (err: any) {
      alert(err.message || "Failed to cancel booking");
    }
  };

  const closeActionSuccessModal = () => {
    setShowActionSuccessModal(false);
    setLastAction(null);
    setActionListingTitle(null);
  };

  if (loading && listings.length === 0) {
    return <div>Loading your listings...</div>;
  }

  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-3xl font-semibold text-gray-900">My Selling</h2>
      </div>

      {error && <div className="text-sm text-red-500">Error: {error}</div>}
      {listings.length === 0 && !loading && (
        <p className="text-sm text-gray-500">
          You have not created any listings yet.
        </p>
      )}

      <div
        className="
          grid gap-4
          grid-cols-1
          sm:grid-cols-2
          md:grid-cols-3
          xl:grid-cols-4
        "
      >
        {listings.map((listing) => {
          const seller = getSellerDisplay(listing.sellerId);

          return (
            <div
              key={listing.id}
              className="card w-75 bg-base-100 card-xl shadow-sm h-full flex flex-col"
            >
              <figure>
                <img
                  src={listing.imageUrl || defaultImage}
                  onError={(e) => (e.currentTarget.src = defaultImage)}
                  className={
                    "mb-3 h-56 w-full rounded-md object-cover transition-all " +
                    (listing.status === "sold"
                      ? "grayscale brightness-75"
                      : "")
                  }
                />
              </figure>

              <div className="card-body flex-1 flex flex-col gap-1 p-3">
                <h2 className="card-title">
                  {listing.title}
                  <div
                    className={`
                      badge ml-2
                      ${
                        listing.status === "available"
                          ? "bg-green-600 text-white"
                          : ""
                      }
                      ${
                        listing.status === "booked"
                          ? "bg-blue-600 text-white"
                          : ""
                      }
                      ${
                        listing.status === "sold"
                          ? "bg-gray-400 text-white"
                          : ""
                      }
                    `}
                  >
                    {listing.status.toUpperCase()}
                  </div>
                </h2>

                <h2 className="card-title">${listing.price}</h2>

                <p className="mt-1 line-clamp-2 text-sm text-gray-600">
                  {listing.description}
                </p>

                {/* Seller avatar + name（其实就是你自己） */}
                <div className="mt-2 flex items-center gap-2">
                  <img
                    src={seller.avatarUrl}
                    alt={seller.name}
                    className="w-6 h-6 rounded-full object-cover"
                  />
                  <span className="text-xs text-gray-500">
                    {seller.name}
                  </span>
                </div>

                {/* buyer 信息（如果有） */}
                {listing.status === "booked" && listing.buyerId && (
                  <p className="mt-1 text-xs text-gray-500">
                    Booked by:{" "}
                    {buyerNameMap[listing.buyerId] ?? listing.buyerId}
                  </p>
                )}

                {/* Details 按钮 */}
                <button
                  type="button"
                  className="text-xs text-gray-500 hover:text-gray-800 underline-offset-2 hover:underline px-0 py-0 mt-1 text-left"
                  onClick={() => setSelectedListing(listing)}
                >
                  Details
                </button>

                {/* 操作按钮区域 / 占位保持高度 */}
                {listing.status === "booked" && (
                  <div className="card-actions justify-end mt-auto gap-2">
                    <button
                      className="btn w-32"
                      onClick={() => handleCancel(listing.id, listing.title)}
                    >
                      Cancel
                    </button>
                    <button
                      className="btn btn-primary w-32"
                      onClick={() => handleSell(listing.id, listing.title)}
                    >
                      Sold
                    </button>
                  </div>
                )}

                {listing.status !== "booked" && (
                  <div className="mt-auto h-6" />
                )}
              </div>
            </div>
          );
        })}
      </div>

      {/* Details 弹窗 */}
      {selectedListing && (() => {
        const seller = getSellerDisplay(selectedListing.sellerId);
        return (
          <div className="modal">
            <div
              className="modal-backdrop"
              onClick={() => setSelectedListing(null)}
            />
            <div className="modal-box max-w-lg">
              <button
                type="button"
                className="modal-close"
                onClick={() => setSelectedListing(null)}
                aria-label="Close"
              >
                ×
              </button>

              <div className="space-y-3">
                <img
                  src={selectedListing.imageUrl || defaultImage}
                  onError={(e) => (e.currentTarget.src = defaultImage)}
                  className="w-full h-64 object-cover rounded-md"
                />

                <div className="flex items-baseline justify-between gap-4">
                  <h3 className="text-lg font-semibold">
                    {selectedListing.title}
                  </h3>
                  <p className="text-lg font-semibold text-gray-900">
                    ${selectedListing.price}
                  </p>
                </div>

                {/* Seller avatar + name in modal */}
                <div className="flex items-center gap-2 text-xs text-gray-500">
                  <img
                    src={seller.avatarUrl}
                    alt={seller.name}
                    className="w-6 h-6 rounded-full object-cover"
                  />
                  <span>{seller.name}</span>
                </div>

                {/* buyer 信息（如果有） */}
                {selectedListing.status === "booked" &&
                  selectedListing.buyerId && (
                    <p className="text-xs text-gray-500">
                      Booked by:{" "}
                      {buyerNameMap[selectedListing.buyerId] ??
                        selectedListing.buyerId}
                    </p>
                  )}

                {/* 完整 description */}
                <p className="text-sm text-gray-800 whitespace-pre-wrap">
                  {selectedListing.description}
                </p>

                <div className="modal-actions justify-end">
                  <button
                    type="button"
                    className="btn"
                    onClick={() => setSelectedListing(null)}
                  >
                    Close
                  </button>
                </div>
              </div>
            </div>
          </div>
        );
      })()}

      {/* Cancel / Sold 成功弹窗 */}
      {showActionSuccessModal && (
        <div className="modal">
          <div className="modal-backdrop" onClick={closeActionSuccessModal} />
          <div className="modal-box">
            <button
              type="button"
              className="modal-close"
              onClick={closeActionSuccessModal}
              aria-label="Close"
            >
              ×
            </button>
            <h3 className="modal-title">
              {lastAction === "cancel"
                ? "Booking cancelled"
                : "Marked as sold"}
            </h3>
            <p className="modal-body-text">
              {lastAction === "cancel" ? (
                <>
                  You have cancelled the booking for{" "}
                  <span className="font-semibold">
                    {actionListingTitle ?? "this listing"}
                  </span>
                  .
                </>
              ) : (
                <>
                  You have marked{" "}
                  <span className="font-semibold">
                    {actionListingTitle ?? "this listing"}
                  </span>{" "}
                  as sold.
                </>
              )}
            </p>
            <div className="modal-actions">
              <button
                type="button"
                className="btn btn-primary"
                onClick={closeActionSuccessModal}
              >
                OK
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
};

export default SellingPage;
